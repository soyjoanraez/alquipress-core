...
class WC_Global_Availability_Data_Store extends WC_Data_Store_WP {
	...
	public function create( &$availability ) {
		...
		$data = array(
			...
			'rule_type'     => $availability->get_rule_type( 'edit' ),
	...
	/**
	 * Get availability rules grouped by date or day.
	 *
	 * @param string $rule_type The rule type ('date_override' or 'weekly').
	 * @param bool   $include_non_bookable Whether to include non-bookable rules (only applies to date_override).
	 * @return array Availability rules grouped by date or day.
	 */
	public function get_time_slots_grouped( $rule_type, $include_non_bookable = true ) {
		global $wpdb;

		// Validate rule type.
		if ( ! in_array( $rule_type, array( 'date_override', 'weekly' ), true ) ) {
			return array();
		}

		$select_field = ( 'date_override' === $rule_type ) ? 'from_date' : 'range_type';
		$group_field  = $select_field;
		$order_field  = $select_field;

		// Build ranges concatenation.
		if ( $include_non_bookable ) {
			$ranges_concat = "GROUP_CONCAT(CONCAT(from_range, '-', to_range, '-', bookable) ORDER BY from_range SEPARATOR ', ') AS ranges";
		} else {
			$ranges_concat = "GROUP_CONCAT(CONCAT(from_range, '-', to_range) ORDER BY from_range SEPARATOR ', ') AS ranges";
		}

		// WHERE clause + parameters.
		$where_clause = 'rule_type = %s';
		$params       = array( $rule_type );

		if ( ! $include_non_bookable ) {
			$where_clause .= " AND bookable = 'yes'";
		}

		$sql = "
			SELECT {$select_field}, {$ranges_concat}
			FROM {$wpdb->prefix}wc_bookings_availability
			WHERE {$where_clause}
