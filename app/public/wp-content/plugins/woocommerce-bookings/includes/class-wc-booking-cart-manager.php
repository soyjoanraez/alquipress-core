 ...
 *
 * Handles cart management for bookings.
 */
class WC_Booking_Cart_Manager {
	 ...
	 *
	 * @return WC_Booking_Cart_Manager
	 */
	...
	public function __construct() {
		...
		// Register structured data filter early so it's available when WooCommerce generates structured data.
	 ...
	 *
	 * @param mixed $passed Boolean passed carry value.
	 * @param mixed $product_id Product ID.
	 * @param mixed $qty Quantity.
	 * @return bool
	 */
	public function validate_add_cart_item( $passed, $product_id, $qty ) {
		if ( WC_Bookings_Core_Compatibility::is_store_api_request() ) {
			return $passed;
		}
	...
	public function cart_loaded_from_session() {
		...
		// Calculate totals once after all items have been removed.
		...
		// Only add the notice if it hasn't been added already.
	 ...
	 *
	 * @throws Exception If in-cart booking is not created.
	 *
	 * @param mixed $cart_item_meta Cart item meta.
	 * @param mixed $product_id Product ID.
	...
	public function add_cart_item_data( $cart_item_meta, $product_id ) {
		...
		if ( ! key_exists( 'booking', $cart_item_meta ) ) {
			$posted_data               = wc_bookings_get_posted_data( $_POST, $product ); // phpcs:ignore WordPress.Security.NonceVerification.Missing
			$cart_item_meta['booking'] = $this->configure_cart_item_data( $posted_data, $product );
		}
	...
	/**
	 * Configure cart item data
	 *
	 * @since 3.0.0
	 *
	 * @throws Exception If in-cart booking is not created.
	 *
	 * @param array              $data Request args. @see wc_bookings_get_posted_data().
	 * @param WC_Product_Booking $product Product.
	 * @return array
	 */
	public function configure_cart_item_data( $data, $product ) {
		if ( ! is_wc_booking_product( $product ) ) {
			return $data;
		}

		$data['_cost'] = WC_Bookings_Cost_Calculation::calculate_booking_cost( $data, $product );
		if ( $data['_cost'] instanceof WP_Error ) {
			throw new Exception( esc_html( $data['_cost']->get_error_message() ) );
		}

		// Create the new booking.
		$new_booking = $this->create_booking_from_cart_data( $data, $product->get_id() );
		if ( ! $new_booking || ! $new_booking->get_id() ) {
			throw new Exception( esc_html__( 'Something went wrong. Please try again.', 'woocommerce-bookings' ) );
		}

		$data['_booking_id'] = $new_booking->get_id();

		// Schedule this item to be removed from the cart if the user is inactive.
		$this->schedule_cart_removal( (int) $new_booking->get_id() );
	 ...
	 *
	 * @param array  $data Booking Cart data.
	 * @param int    $product_id Product ID.
	 * @param string $status Booking status.

	 * @return WC_Booking
	 */
	private function create_booking_from_cart_data( $data, $product_id, $status = 'in-cart' ) {
		// Create the new booking.
		$new_booking_data = array(
			...
			'all_day'        => $data['_all_day'],
			'local_timezone' => $data['_local_timezone'],
		);

		// Check if the booking has resources.
		if ( isset( $data['_resource_id'] ) ) {
			$new_booking_data['resource_id'] = $data['_resource_id'];
		}

		// Checks if the booking allows persons.
		if ( isset( $data['_persons'] ) ) {
			$new_booking_data['persons'] = $data['_persons']; // Count of persons making booking.
		}
	...
	/**
	 * Modify structured data for bookable-type products.
	 *
	...
	public function structured_product_data( $data, $product ) {
		if ( is_object( $product ) && is_wc_booking_product( $product ) ) {
			...
			if ( isset( $data['priceSpecification']['price'] ) ) {
				$data['priceSpecification']['price'] = $price;
			}
