...
class WC_Product_Data_Store_CPT extends WC_Data_Store_WP {
	...
	*/
	...
	public function read( &$product ) {
		...
		if ( ! $product->get_id() || ! $post_object || 'product' !== $post_object->post_type ) {
			throw new Exception( esc_html__( 'Invalid product.', 'woocommerce-bookings' ) );
		}
	...
	*/
	...
	public function get_related_products( $cats_array, $tags_array, $exclude_ids, $limit, $product_id ) {
		global $wpdb;
		/**
		 * Filter the related products query.
		 *
		 * @since 2.2.3
		 *
		 ...
		 * @return string
		 */
		return $wpdb->get_col( implode( ' ', apply_filters( 'woocommerce_product_related_posts_query', $this->get_related_products_query( $cats_array, $tags_array, $exclude_ids, $limit + 10 ), $product_id ) ) ); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
	}
