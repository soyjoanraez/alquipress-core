...
/**
 * WC_Bookings_Create class.
 *
 * Handles the create new bookings page functionality.
 */
	...
	public function output() {
		...
		try {
			if ( ! empty( $_POST ) && ! check_admin_referer( 'create_booking_notification' ) ) {
				throw new Exception( esc_html__( 'Error - please try again', 'woocommerce-bookings' ) );
			}
			...
			if ( ! empty( $_POST['create_booking'] ) ) {
				...
				if ( ! $bookable_product_id ) {
					throw new Exception( esc_html__( 'Please choose a bookable product', 'woocommerce-bookings' ) );
				}

				if ( 'existing' === $booking_order ) {
					if ( ! isset( $_POST['booking_order_id'] ) ) {
						throw new Exception( __( 'Order ID not provided', 'woocommerce-bookings' ) );
					}
					...
					if ( ! $booking_order || ! WC_Booking_Order_Compat::is_shop_order( $booking_order ) ) {
						throw new Exception( esc_html__( 'Invalid order ID provided', 'woocommerce-bookings' ) );
					}
				}
				...
				if ( 'new' === $booking_order ) {
					...
					if ( ! $order_id ) {
						throw new Exception( esc_html__( 'Error: Could not create order', 'woocommerce-bookings' ) );
					}
