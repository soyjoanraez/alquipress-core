 ...
 * WC_Bookings_Menus class.
 *
 * Handles admin menu functionality for bookings.
 */
		...
		if ( ! empty( $_POST ) && check_admin_referer( 'send_booking_notification' ) ) {
			...
			try {
				if ( ! $notification_product_id ) {
					throw new Exception( esc_html__( 'Please choose a product', 'woocommerce-bookings' ) );
				}

				if ( ! $notification_message ) {
					throw new Exception( esc_html__( 'Please enter a message', 'woocommerce-bookings' ) );
				}

				if ( ! $notification_subject ) {
					throw new Exception( esc_html__( 'Please enter a subject', 'woocommerce-bookings' ) );
				}
