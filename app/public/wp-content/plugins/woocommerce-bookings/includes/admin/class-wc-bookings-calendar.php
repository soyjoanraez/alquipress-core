...
class WC_Bookings_Calendar {
		...
		foreach ( $events as $event ) {
			if ( 'WC_Booking' === get_class( $event ) ) {
				...
				foreach ( $days as $day ) {
					...
					$title       = ! empty( $availability->get_title() ) ? $availability->get_title() : __( 'Unavailable', 'woocommerce-bookings' );
	...
	protected function get_booking_data( $event, $check_date = null ) {
		...
		if ( 'WC_Booking' === get_class( $event ) ) {
			...
			$event_title  = ! empty( $availability->get_title() ) ? $availability->get_title() : __( 'Unavailable', 'woocommerce-bookings' );
	...
	public function random_color() {
		return sprintf( '#%06X', mt_rand( 0, 0xFFFFFF ) ); // phpcs:ignore WordPress.WP.AlternativeFunctions.rand_mt_rand
	}
