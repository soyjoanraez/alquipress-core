...
class WC_Bookings_Templates {
	...
	public function __construct() {
		...
		* Show upgrade notice.
		*/
		add_action( 'upgrader_process_complete', array( $this, 'woocommerce_bookings_upgrade' ), 10, 2 );
		add_action( 'init', array( $this, 'show_upgrade_notice' ), 10 );
	}
	...
	public function show_upgrade_notice() {
		...
		// Bail early if user dismissed the upgrade notice.
		if ( WC_Admin_Notices::user_has_dismissed_notice( 'woocommerce_bookings_upgrade' ) ) {
			return false;
		}
		...
		$notice_html = sprintf(
			...
			esc_html__(
				'WooCommerce Bookings now includes bookable product templates that can be imported to more easily set up new bookable products. %1$sClick here%2$s to learn more about them.',
	 ...
	 * @since 2.0.0
	 */
	public function woocommerce_bookings_upgrade( $upgrader_object, $options ) {
		// Early return if upgrade notice has already been shown.
		if ( 'shown' === get_option( WC_BOOKINGS_UPGRADE_NOTICE_KEY ) ) {
			return;
		}

		// Early return if 'plugins' are not updated or WooCommerce Bookings is not one of the updated plugins.
		if ( ! isset( $options['plugins'] ) || ! in_array( 'woocommerce-bookings/woocommerce-bookings.php', $options['plugins'], true ) ) {
			return;
		}

		// Flag to trigger upgrade notice.
		update_option( WC_BOOKINGS_UPGRADE_NOTICE_KEY, 'shown' );
	}
