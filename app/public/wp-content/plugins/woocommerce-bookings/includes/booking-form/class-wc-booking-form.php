...
class WC_Booking_Form {
	...
	public function get_end_times( $blocks, $start_date_time = '', $intervals = array(), $resource_id = 0, $from = 0, $to = 0, $check = false ) {
		...
		if ( ! array_key_exists( $start_time, $blocks ) ) { // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
			return $data;
		}
		...
		if ( $check ) {
			...
			return ! is_wp_error( $available_blocks ) && $available_blocks && array_key_exists( $start_time, $blocks ); // phpcs:ignore WordPress.PHP.StrictInArray.MissingTrueStrict
		}
