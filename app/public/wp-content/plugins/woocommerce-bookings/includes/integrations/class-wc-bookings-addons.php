...
class WC_Bookings_Addons {
	...
	public function addons_show_grand_total( $show_grand_total, $product ) {
		if ( is_wc_booking_product( $product ) && defined( 'WC_PRODUCT_ADDONS_VERSION' ) && version_compare( WC_PRODUCT_ADDONS_VERSION, '3.0', '<' ) ) {
	...
	public function addon_options( $post, $addon, $loop ) {
		...
		<div class="<?php echo esc_attr( $css_classes ); ?>">
			<div class="addon_wc_booking_person_qty_multiplier">
				...
				<?php echo wc_help_tip( __( 'Only applies to options which use quantity based prices.', 'woocommerce-bookings' ) ); // phpcs:ignore WordPress.Security.EscapeOutput.OutputNotEscaped ?>
				</label>
			</div>
	...
	public function adjust_price( $bool, $cart_item ) {
		if ( is_a( $cart_item['data'], 'WC_Product' ) && is_wc_booking_product( $cart_item['data'] ) ) {
			return false;
		}
		return $bool;
	}
