...
class WC_Deposits_Cart_Manager {
	...
	public function validate_add_cart_item( $passed, $product_id, $qty, $variation_id = 0, $variations = array(), $cart_item_data = array() ) {
		...
		if ( ( 'yes' === $wc_deposit_option || WC_Deposits_Product_Manager::deposits_forced( $product_id ) ) && 'plan' === WC_Deposits_Product_Manager::get_deposit_type( $product_id ) ) {
			$available_plans = WC_Deposits_Plans_Manager::get_plan_ids_for_product( $product_id );

			if (
				( ! is_array( $available_plans ) || count( $available_plans ) !== 1 )
				&& ! in_array( $wc_deposit_payment_plan, $available_plans, true )
			) {
	...
	public function add_cart_item_data( $cart_item_meta, $product_id, $variation_id ) {
		...
		if ( 'yes' === $wc_deposit_option || WC_Deposits_Product_Manager::deposits_forced( $product_id ) ) {
			...
			if ( 'plan' === WC_Deposits_Product_Manager::get_deposit_type( $product_id ) ) {
				$available_plans = WC_Deposits_Plans_Manager::get_plan_ids_for_product( $product_id );
				if ( is_array( $available_plans ) && count( $available_plans ) === 1 ) {
					$wc_deposit_payment_plan = reset( $available_plans );
				}
	...
	public function add_to_cart_text( $text, $product = null ) {
		...
		if ( WC_Deposits_Product_Manager::deposits_forced( $product->get_id() ) ) {
			...
			$available_plans = WC_Deposits_Plans_Manager::get_plan_ids_for_product( $product->get_id() );

			if ( ! is_array( $available_plans ) || 1 === count( $available_plans ) ) {
				// Show default text if there are no options to select.
				return $text;
			}
		}
	...
	public function add_to_cart_url( $url, $product = null ) {
		...
		if ( WC_Deposits_Product_Manager::deposits_forced( $product_id ) ) {
			...
			if ( ! is_array( $available_plans ) || 1 === count( $available_plans ) ) {
				// Use default url if there are no options to select.
				return $url;
			}
		}
	...
	public function remove_add_to_cart_class( $link, $product ) {
		...
		if ( WC_Deposits_Product_Manager::deposits_enabled( $product_id ) ) {
			$deposit_type = WC_Deposits_Product_Manager::get_deposit_type( $product_id );
			// If Deposits are optional, remove the add to cart class(Disable AJAX add to cart).
			if ( ! WC_Deposits_Product_Manager::deposits_forced( $product_id ) ) {
			...
			if ( 'plan' === $deposit_type ) {
				...
				// If multiple payment plans are available, remove the add to cart class(Disable AJAX add to cart).
				if ( is_array( $available_plans ) && 1 !== count( $available_plans ) ) {
					$link = str_replace( 'add_to_cart_button', '', $link );
				}
			}
	...
	public function ajax_add_to_cart_supports( $supports, $feature, $product ) {
		...
		// The purchaser needs to select from a range of options.
		return false;
	}
}
